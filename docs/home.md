...
**snac** означает "Social Networks Are Crap" (социальные сети это отстой).
...
**Приватная лента** или просто лента — это частная, защищенная паролем область сервера **snac**, где пользователь взаимодействует с остальной частью Fediverse.
...
Добавляет в очередь сообщение `Create + Note` для всех подписчиков. Если аргумент **text** задан как `-e`, будет вызван внешний редактор, указанный в переменной окружения `EDITOR`, для подготовки сообщения; если он равен `-` (одинокому дефису), содержимое поста будет считано из *stdin*. Остальные аргументы командной строки интерпретируются как медиафайлы, прикрепляемые к посту. Переменная окружения `LANG` (если определена) используется в качестве языка поста. Можно указать необязательный URL поста в Fediverse, перед которым стоит ключ `-r`, чтобы эта заметка являлась ответом на указанный пост. Если перед датой в формате `YYYYmmddHHMMSS` указать `-d`, заметка будет запланирована на указанное время вместо немедленной публикации (эта дата должна быть в том же часовом поясе, который пользователь настроил в веб-интерфейсе).
...
- **max_failing_days**  
Максимальное количество дней, в течение которых инстанс считается недоступным после непрерывных неудачных исходящих сообщений (по умолчанию: 15). Тем не менее, при обнаружении любой входящей активности от ранее недоступного инстанса, этот счётчик для него сбрасывается.
