...
Добавляет в очередь сообщение `Create + Note` для всех подписчиков. Если аргумент *text* равен `-e`, будет вызван внешний редактор, определённый переменной окружения `EDITOR`, чтобы подготовить сообщение; если он равен `-` (одиночное тире), содержимое поста будет прочитано из стандартного ввода. Остальные аргументы командной строки рассматриваются как медиафайлы, которые нужно прикрепить к посту. Переменная окружения `LANG` (если она определена) используется как язык поста. Также можно указать необязательный URL на пост в Fediverse, в ответ на который будет отправлена эта заметка.
