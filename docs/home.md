...
- `actor`: идентификатор актора, инициировавшего действие  
- `target`: идентификатор целевого актора  
- `date`: дата уведомления  
- `message`: полный объект действия в формате JSON (как определено в ActivityPub)  
- `objid`: идентификатор объекта (извлекается из поля `message`, может быть null)  
- `type`: тип действия (извлекается из `message`)  
- `utype`: подтип действия (извлекается из `message`, может быть null)  
