...
- **http_headers**  
Если по какой-либо причине вам необходимо добавить дополнительные заголовки HTTP-ответа, вы можете заполнить этот объект необходимыми парами заголовок/значение. Например, для повышения безопасности XSS вы можете установить для заголовка `Content-Security-Policy` значение `script-src ;` чтобы быть полностью уверенным, что никакой JavaScript не выполняется.
...
Начиная с версии `2.50`, контент сообщений можно фильтровать с помощью регулярных выражений. Это оружие массового поражения можно записать в файл `filter_reject.txt` в базовом каталоге сервера, по одному в каждой строке; если этот файл существует, содержимое всех сообщений  (после удаления HTML-тегов) будет сопоставляться с этими регулярными выражениями, одно за другим и любое совпадение приведет к отклонению сообщения. Если вы не знаете о регулярных выражениях, не используйте эту опцию (или узнайте о них в каком-нибудь руководстве, их там тысячи), так как вы и ваши пользователи можете начать терять сообщения. Кроме того, учитывая, что каждая реализация регулярных выражений поддерживает свой набор функций, рассмотрите возможность прочтения документации по той реализации, которая реализована в вашей системе.
