...
- **spawn_sendmail**  
Начиная с версии `2.76`, уведомления по электронной почте отправляются через прямое SMTP-подключение вместо вызова `/usr/sbin/sendmail` для каждого сообщения. Установив это значение в `true`, можно восстановить классическое поведение.

- **smtp_url**  
URL-адрес SMTP-сервера, который будет использоваться для отправки уведомлений по электронной почте. По умолчанию это `smtp://localhost`, что подразумевает полностью настроенный SMTP-сервер, работающий на том же хосте. Если сервер работает на нестандартном порту, можно указать номер порта, например, `smtp://mail.example.com:587`.
